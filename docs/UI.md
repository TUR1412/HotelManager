...
- **多巴胺反馈**：操作按钮触发 confetti + shimmer + toast
- **主题切换**：支持深色/浅色一键切换（记忆用户偏好，并尊重系统偏好）
- **快捷键**：`I` 导入快照 / `G` 生成日报 / `T` 切换主题

## 主题系统

- **深色 / 浅色**：通过 CSS 变量与 `data-theme` 控制视觉基底。
- **默认策略**：优先读取本地偏好（`localStorage`），否则跟随系统偏好。
- **目标**：不依赖任何框架与构建工具，保持“单文件可打开”。
...
导入成功后，页面会基于快照实时计算并刷新：
- KPI 卡片
- 最新预订列表（reserved，最多 3 条）
- 今日概览（入住率 / 今日到店 / 今日离店）
- 房态总览（可售 / 在住 / 维修）
- 收入曲线（近 14 日房晚收入，按 `price_per_night_cents` 聚合）

同时支持一键生成并导出 **Markdown 日报**（本地生成，不联网）。
